"""
Strict English CS HR Auto-Responder.
Strict Geofence: Excludes 湖南 / 长沙 / 怀化 / 洪江.
Target Filter: Only communicates with HRs of 英语客服 / 英文客服 / 海外客服 / 跨境客服.
Reads latest HR message, generates English CS high-EQ reply, types and sends it.
"""
import sys
import os
import subprocess
...
async def main():
    print("\n" + "="*70)
    print("🎯 BOSS 直聘【严格限定·仅回复英语客服/海外客服 HR】启动")
    print("🛡️ 湖南本地 100% 隔离 | 📄 绑定简历: 杨春_个人求职简历.pdf")
    print("="*70 + "\n", flush=True)
    ...
    async with async_playwright() as p:
        ...
        print(f"\n3. 📋 扫描到 {len(conv_list)} 个会话，正在精准筛选【英语客服/海外客服】目标：\n", flush=True)
        
        replied_count = 0
        for c in conv_list:
            item_text = c["text"]
            
            # 严格过滤：非英语客服/海外客服或湖南本地一律跳过
            if not is_english_cs_conversation(item_text):
                print(f"   ⏩ 跳过非英语客服/非测试会话: {item_text[:45]}", flush=True)
                continue
                
            print("\n" + "─"*65)
            print(f"🎯 【命中安全英语客服目标】: {item_text[:65]}")
            print("─"*65, flush=True)
            ...
            # 如果 HR 尚未发消息，我们主动跟进一条英语客服的沟通消息；如果 HR 发了消息，我们针对性应答
            if last_hr_msg:
                print(f"   💬 【捕获到英语客服 HR 最新消息】: \"{last_hr_msg}\"", flush=True)
                reply_text = generate_english_cs_reply(last_hr_msg)
            else:
                print("   💬 该英语客服会话处于建立状态，正在发送专属跟进消息...", flush=True)
                reply_text = "您好！关注到贵司的英语客服岗位，我的英语听说读写能力良好，能熟练处理英文工单与客户沟通，请问方便进一步了解下岗位职责吗？"
            ...
            try:
                if await input_box.is_visible():
                    ...
                    print("   🎉 ✅ 消息已成功打字并回车发送至英语客服 HR！", flush=True)
                    replied_count += 1
                    await asyncio.sleep(2.0)
            except Exception as e:
                print(f"   ⚠️ 输入框交互提示: {e}", flush=True)
                
            await page.screenshot(path=str(screenshots_dir / f"english_cs_convo_{c['idx']+1}_replied.png"))
            await page.screenshot(path=str(screenshots_dir / "english_cs_reply_verified.png"))
            
        print("\n" + "="*70)
        print(f"🎉 【测试完成！共成功回复并跟进了 {replied_count} 位【英语客服/海外客服】HR！】")
        print("="*70 + "\n", flush=True)
